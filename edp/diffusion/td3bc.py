...
class TD3BC(Algo):
  ...
  def __init__(self, cfg, policy, qf, vf, policy_dist):
    ...
    #self.max_action = policy.max_actionself.diffusion.action_dim
  ...
  def _train_step_td3(
    ...
    def policy_loss_fn(params, tgt_params, rng):
      ...
      predicted_action = self.policy.apply(
        #tgt_params['policy'], observations, training=True,# rngs=rng
        params['policy'], observations, training=True,# rngs=rng
      )
